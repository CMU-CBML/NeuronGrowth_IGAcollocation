...
% mu = -2.106;
% sigma = 28.014;
quantiles = [1.736,11.029,29.323,68.394];
...
%         fprintf('Phi NR Iter: %.2d -> max residual: %.2d\n',ind_check, max_phi_R);
    ...
    elseif ( iter>=iter_stage3_begin && iter < iter_stage45_begin)
        ...
        for k = 1:L.NumObjects
            ...
            if (mod(iter,50) == 0)
                growthAngle = getExpRelAng(quantiles);
                [cx(k),cy(k)] = expDist(centroids(k,1),centroids(k,2),mx,my,growthAngle);
            end
        ...
        if(mod(iter,png_plot_invl) == 0)
            ...
            title(sprintf('ox:%.2d,oy:%.2d,x:%.2d,y:%.2d,gA:%2d',cx,cy,mx,my,growthAngle));
            axis square;
            colorbar;
            hold on;
