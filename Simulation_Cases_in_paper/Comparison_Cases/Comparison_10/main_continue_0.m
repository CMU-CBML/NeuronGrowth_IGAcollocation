...
initial_LAPpk = load('./data/initial_LAPpk_ 1.mat').initial_LAPpk;
% resizing initial_LAPpk cause 
sz = length(phi);
initial_LAPpk = kqExpandDomainForInitialLAPpk(sz,initial_LAPpk);
sum_lap_phi = sum(initial_LAPpk.*initial_LAPpk);
expd_state = 0;

if iter <=5000
    CX = struct('c1',[lenu,lenu/5,0],'c2',[0,0,0,lenu,lenu*2/3]);
    CY = struct('c1',[lenv,0,lenv/2],'c2',[0,lenv/3,lenv,lenv,lenv]);

else
    CX = struct('c1',[lenu,lenu/5,0,lenu*2/3],'c2',[0,0,0,lenu,lenu/3,lenu]);
    CY = struct('c1',[lenv,0,lenv/2,0],'c2',[0,lenv/3,lenv,lenv,lenv,lenv/2]);
end
...
%         if iter <=5000
% %             CX = {lenu*4/5,lenu/3,0;
% %                 0,0,0,lenu,lenu*2/3};
% %             CY = {lenv,0,lenv/2;
% %                 0,lenv/3,lenv,lenv,lenv};
%             CX = struct('c1',[lenu*4/5,lenu/3,0],'c2',[0,0,0,lenu,lenu*2/3]);
%             CY = struct('c1',[lenv,0,lenv/2],'c2',[0,lenv/3,lenv,lenv,lenv]);
% 
%         else
% %             CX = [lenu*4/5,lenu/3,0,lenu*2/3;
% %                 0,0,0,lenu,lenu/3,lenu];
% %             CY = [lenv,0,lenv/2,0;
% %                 0,lenv/3,lenv,lenv,lenv,lenv/2];
%             CX = struct('c1',[lenu*4/5,lenu/3,0,lenu*2/3],'c2',[0,0,0,lenu,lenu/3,lenu]);
%             CY = struct('c1',[lenv,0,lenv/2,0],'c2',[0,lenv/3,lenv,lenv,lenv,lenv/2]);
%         end
%     end
    ...
    elseif iter >= 1000
        ...
        max_y = [];
%         numCell = 2;
        for c = 1:L.NumObjects
            
            tip = sum_filter(phi_id.*(ID==c),1);
